...
where $S_t$ is the seasonal component, $T_t$ is the trend, $\varepsilon_t$ is the remainder, all at time $t$.

- Necessary but not sufficient condition: Make $\varepsilon_t$ "white noise", i.e. $\varepsilon_t \stackrel{iid}\sim \mathcal{N}(0, \sigma^2)$
...
4. The remainder component is calculated by subtracting the estimated seasonal and trend-cycle components: $\hat{\varepsilon}_t = Y_t - \hat{T}_t - \hat{S}_t$
...
Actually we already wrote this: $\varepsilon_t \stackrel{iid}\sim \mathcal{N}(0, \sigma^2)$
:::
...
What if $Y_t = \mu + \varepsilon_t$?

What if $Y_t = \mu_t + \varepsilon_t = \beta_0 + \beta_1\cdot t + \varepsilon_t$?
...
models_fit <- models_fit |> forecast(calves_te)
