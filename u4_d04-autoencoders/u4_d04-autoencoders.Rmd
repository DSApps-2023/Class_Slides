...
In terms of $\gamma = \log\sigma^2$, this means: $z = \varepsilon \cdot \frac{\text{exp}(\gamma)}{2} + \mu$
...
kl_loss = -0.5 * K.sum(
  ...
  axis=-1)
reconstruction_loss = MeanSquaredError(inputs, outputs) * 784
vae_loss = K.mean(reconstruction_loss + kl_loss)

variational_ae.add_loss(vae_loss)
variational_ae.compile(optimizer='adam')

history = variational_ae.fit(X_train, X_train, epochs=20, verbose=1)
```
...
```{python VAE-recon, out.width="80%", eval=FALSE}
...
shoe3 = variational_decoder.predict(shoe_encoded +np.random.normal(scale=5))[0]
```

---

```{python VAE-Shoe-decoded, out.width = "80%", eval=FALSE}
fig, axes = plt.subplots(1, 3, figsize=(6, 3))
axes[0].imshow(shoe1, cmap="binary")
axes[1].imshow(shoe2, cmap="binary")
axes[2].imshow(shoe3, cmap="binary")
...
```{python VAE-Mean-decoded, out.width = "80%", eval=FALSE}
...
```{python VAE-Random-decoded, out.width = "80%", eval=FALSE}
X_test_compressed = variational_encoder.predict(X_test)
mins = X_test_compressed.min(axis=0)
maxs = X_test_compressed.max(axis=0)
