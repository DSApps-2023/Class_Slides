...
- Performance in ML:
  ...
  - Models which suffer from Multicollinearity (which?)
...
- Let $\mu_i$ be the $p$-dimensional mean vector for each individual $i$

- then write the FA model: $x_i = Wt_i + \mu_i + \varepsilon_i$
...
Conditionally, $x_i | t_i \sim N(Wt_i + \mu_i, \Psi)$.

Marginally, $x_i \sim N(\mu_i, V)$, where $V_{p \times p} = WW' + \Psi$ (communality + uniqueness).

Write the marginal log-likelihood: $L(\mu, W, \Psi | X) = -\frac{n}{2}[p\ln{(2\pi)} + \ln|V| + \text{tr}(V^{-1}S)]$,

where $S$ is the covariance matrix of $X$: $S = \frac{1}{n}\sum_{i = 1}^{n} (x_i - \mu_i)(x_i - \mu_i)'$
