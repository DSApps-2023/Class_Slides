...
- It's just a set of if-else statements my ~~7~~ ~~8~~ ~~9~~ 10 y/o can get
...
okcupid2 <- okcupid2 %>%
  ...
  mutate(across(essay0:essay9, list("len" = ~log(str_length(.x) + 0.016)))) %>%
...
# train_idx <- okcupid2 %>% filter(!id %in% test_idx) %>% slice_sample(n = 8000) %>% pull(id)
...
.insight[
`r emoji("bulb")` Can you think of other imputation strategies?
]
...
.insight[
`r emoji("bulb")` What if the best split isn't binary?
]
...
.insight[
`r emoji("bulb")` What could be an issue with a categorical variable with many levels?
]

- a categorical predictor $v$ is nominated for splitting the current node
  - option 1: use dummy variables, turning each level into a 2-level 0/1 category variable
  - option 2: order levels by some criterion like $\overline{y_j}$ and treat $v$ as continuous from here on
...
.warning[
`r emoji("warning")` There is a $c_p$ parameter you can pass to `rpart` while training like so:
...
.insight[
`r emoji("bulb")` How would this profile look for a couple of very correlated predictors?
]
...
```{r, echo=FALSE}
# New laptop, saw no reason to install Java, apologies
RMSE <- 0.374
CORR <- 0.392
glue("RMSE: {format(RMSE, digits = 3)}
     CORR: {format(CORR, digits = 3)}")
...
.insight[
`r emoji("bulb")` What is troubling about variable importance?
]
---
...
That is why GBT (or GBM) in general is called an additive model.

.insight[
`r emoji("bulb")` Where does "Gradient" come from?
]
...
.insight[
`r emoji("bulb")` Do you see the difference in the profile of importance between RF and GBT?
]
---
...
.warning[
`r emoji("warning")` Note that `xgboost` only accepts numeric input, that is why we use `data.matrix()`. Perhaps the order of the factors could be improved before they are turned into integers.
]
...
# train_idx <- okcupid3 %>% filter(!id %in% test_idx) %>% slice_sample(n = 10000) %>% pull(id)
...
.insight[
`r emoji("bulb")` What would be the accuracy? Recall (sensitivity, specificity)? Precision (PPV, NPV)?
]
...
For example if `age` is candidate in splitting `pets`:
