...
Our two previous modeling attempts, with [Modeling in the Tidyverse](https://dsapps-2023.github.io/Class_Slides/u1_d05-modeling/u1_d05-modeling.html) and in [The Trees](https://dsapps-2023.github.io/Class_Slides/u3_d01-trees/u3_d01-trees.html) where problematic.
...
- Not thinking about Multicollinearity where it hurts (regression)
...
- But resampling should also be made for:
  ...
  - Feature selection .font80percent[(Should I use 2nd-order interactions here?)]
...
.insight[
`r emoji("bulb")` &ast; How many resamples?
...
.insight[
`r emoji("bulb")` What is the probability of entering a fold (at least once)?
]
...
You: penalized logistic regression it is `r emoji("smirking face")`
...
.insight[
`r emoji("bulb")` We assume these decisions are additive or independent of each other. Does it have to be and what can we do if we suspect not? (Remember we are Statisticians!)
]
...
.insight[
`r emoji("bulb")` How would you asses the missing data mechanism in your dataset?
]
...
```{r Ok-Miss1, out.width="100%", fig.asp=0.5, warning=FALSE}
...
ok_tr_missing %>%
  ...
  labs(x = "", y = "") + theme_light() + theme(axis.text.y=element_blank(), axis.ticks.y=element_blank(), axis.text.x=element_text(angle = 45, hjust = 1))
```
...
.font80percent[For educational purposes] Applying (two) recipes to all splits with `prepper()`:
...
```{r Wset-Autoplot, out.width="90%", fig.asp=0.5}
library(ggrepel)
...
```{r, Ok-Miss9, out.width="85%", fig.asp=0.5}
res_df <- wset %>%
  unnest(result) %>%
...
Let's move on to 2 x 2 recipes, adding 2 more recipes for treating the `income` variable with mean imputation, with `step_unknown()` and `step_impute_mode()`.
...
cv_res_missing_meds <- cv_res_missing %>%
  separate_wider_delim(wflow_id, "_", names = c("categorical", "income", "mod")) %>%
  group_by(income, categorical) %>%
  summarise(auc = mean(auc), id = "All")
```

```{r Ok-Miss10, out.width="90%", fig.asp=0.5}
cv_res_missing %>%
  separate_wider_delim(wflow_id, "_", names = c("categorical", "income", "mod")) %>%
  ggplot(aes(income, auc, group = id, color = id)) +
  geom_line() + geom_line(data = cv_res_missing_meds, color = "black", alpha = 0.2, linewidth = 2) +
...
rec_miss_unk_none_len <- recipe(pets ~ ., data = ok_tr_missing2) %>%
  step_rm(income, starts_with("isna")) %>%
...
cv_res_missing <- wset %>%
  ...
  separate_wider_delim(wflow_id, "_", names = c("categorical", "income", "essay", "mod"))
```
...
.insight[
`r emoji("bulb")` So what would you do?
]
...
.font80percent[(We already gave a decent review in [last class](https://dsapps-2023.github.io/Class_Slides/u3_d02-classtopics/u3_d02-classtopics.html))]
...
cv_res_imbalance %>%
  ggplot(aes(recipe, auc, group = id, color = id)) +
    geom_line() + geom_line(data = cv_res_imbalance_meds, color = "black", alpha = 0.2, linewidth = 2) +
...
- Text Features
- .font80percent[Won't talk but God knows this is on my mind, e.g. [Simchoni and Rosset (2021)](https://papers.nips.cc/paper/2021/hash/d35b05a832e2bb91f110d54e34e2da79-Abstract.html)]
...
.insight[
`r emoji("bulb")` How would you cluster `location`?
]
...
.insight[
`r emoji("bulb")` Why would this work in spite of collisions?

What is still a major disadvantage? Hint: `r emoji("present")`.
]

---

<img src = "images/teacher_fired.png" style = "width: 80%">

.font80percent[
[Bill Turque, 2012, Washington Post](https://www.washingtonpost.com/local/education/creative--motivating-and-fired/2012/02/04/gIQAwzZpvR_story.html)
]
...
See our `word2vec` detour when we learned about [Networks Community Detection](https://dsapps-2023.github.io/Class_Slides/u2_d04-community/u2_d04-community.html).
...
- By default it will accept `count_functions` list from the [`textfeatures`](https://textfeatures.mikewk.com/reference/textfeatures.html) package by Mike Kearney, which gets you basic counts like `n_words`, `n_digits` etc.
...
rec_eng_notext <- recipe(pets ~ ., data = ok_tr_eng3) %>%
  ...
  step_log(income, offset = 1) %>%
  step_impute_mean(income) %>%
  step_other(all_nominal_predictors(), -has_role("discarded"), other = "other1") %>%
...
```{r, echo=FALSE}
cv_res_eng <- read_rds("../data/Ok_eng_cv_res_auc2.rds")
...
full_features <- function(df) {
  ...
  diets_keep <- c("anything", "vegan", "vegetarian")
  ...
  df2 <- handle_essays(df) %>%
    mutate(
      essays = pmap_chr(
        list(essay0, essay1, essay2, essay3, essay4, essay5,
             essay6, essay7, essay8, essay9), str_c),
      offspring_no_kids = str_detect_yesno(offspring, "t have kids"),
      offspring_one_kid = str_detect_yesno(offspring, "has a kid"),
      offspring_many_kids = str_detect_yesno(offspring, "has kids"),
      offspring_wants_kids = str_detect_yesno(offspring,
                                              "wants them|want them|wants more|want more|wants kids|want kids"),
      religion_not2serious = str_detect_yesno(religion, "not too serious"),
      religion_laughing = str_detect_yesno(religion, "laughing"),
      religion_somewhat = str_detect_yesno(religion, "somewhat"),
      religion = case_when(
      ...
      ethnicity = ifelse(ethnicity == "hispanic / latin", "hispanic", ethnicity),
      body_type_perfect = ifelse(body_type %in% not_perfect, 0, 1),
      diet = map_chr(diet, narrow_category),
      diet = ifelse(diet %in% diets_keep, diet, "other"),
      sign_fun = str_detect_yesno(sign, "fun"),
      sign_not_matter = str_detect_yesno(sign, "but it does"),
      sign_matters = str_detect_yesno(sign, "matters"),
      sign_na = ifelse(is.na(sign), 1, 0),
      speaks_spanish = str_detect_yesno(speaks, "spanish"),
      speaks_french = str_detect_yesno(speaks, "french"),
      speaks_german = str_detect_yesno(speaks, "german"),
      speaks_chinese = str_detect_yesno(speaks, "chinese"),
      speaks_n_fluently = str_count_na(speaks, "fluently"),
      speaks_n_okay = str_count_na(speaks, "okay"),
      speaks_n_poorly = str_count_na(speaks, "poorly"),
      ...
      job = ifelse(job == "other", "other_job", job),
      location = case_when(
    ...
    mutate(across(where(is.character), as.factor)) %>%
    select(-speaks, -sign, -offspring, -matches("^essay[0-9]"))
...
```{r, echo=FALSE}
cv_res_commonsense <- read_rds("../data/Ok_cs_cv_res_auc4.rds")
...
cv_res_eng %>%
  ...
  ggplot(aes(recipe, auc, group = id, color = id)) +
  geom_line() + geom_line(data = cv_res_eng_meds %>% bind_rows(cv_res_cs_meds), color = "black", alpha = 0.2, linewidth = 2) +
...
ggplot(ok_tr_eng[!is.na(ok_tr_eng$income), ]) +
  ...
  geom_line(aes(age, mod_lm_bs$fit), color = "red", linewidth = 2, alpha = 0.4) +
  theme_light()
```
...
ggplot(ok_tr_eng[!is.na(ok_tr_eng$income), ]) +
  geom_point(aes(age, log(income))) +
  geom_line(aes(age, mod_lm_bs2$fit), color = "red", linewidth = 2, alpha = 0.4) +
  theme_light()
```
...
.warning[
`r emoji("warning")` No guarantee for improvement in performance! In fact...
]

.insight[
`r emoji("bulb")` What is an alternative to reduce dimensionality?
]
...
.insight[
`r emoji("bulb")` How would you visualize an interaction between two continuous features? Two categorical? Three?...
]
...
So all two-way interactions would mean adding... over 8K features. All three-way interactions would mean adding... over 350K features.

.insight[
`r emoji("bulb")` What danger comes to mind? How would you defend yourself?
...
5. All two-way interactions `r emoji("sunglasses")` .font80percent[(with a `step_nzv()` though)]
...
```{r, echo=FALSE}
all_ints <- read_rds("../data/Ok_interaction_fdr3.rds")
```
...
```{r Ethnicity-Interaction, out.width="80%", fig.asp=0.5}
main_data %>% group_by(offspring_no_kids, offspring_wants_kids) %>%
    summarise(pct_cats = mean(pets == "cats"), n = n()) %>% ungroup() %>%
    mutate(across(c("offspring_no_kids", "offspring_wants_kids"), as.factor)) %>%
    ggplot(aes(offspring_no_kids, pct_cats, color = offspring_wants_kids)) +
    geom_line(aes(group = offspring_wants_kids)) + geom_point(aes(size = n)) +
    scale_y_continuous(labels = percent_format()) +
    theme_light()
```

.insight[
`r emoji("bulb")` Try stating what you see in words.
]

---

```{r Fluently-Interaction, out.width="90%", fig.asp=0.5}
main_data %>% group_by(ethnicity_white, job_hardware) %>%
    summarise(pct_cats = mean(pets == "cats"), n = n()) %>% ungroup() %>%
    mutate(across(c("ethnicity_white", "job_hardware"), as.factor)) %>%
    ggplot(aes(ethnicity_white, pct_cats, color = job_hardware)) +
    geom_line(aes(group = job_hardware)) + geom_point(aes(size = n)) +
    scale_y_continuous(labels = percent_format()) +
    theme_light()
```
...
```{r Religion-Interaction, out.width="90%", fig.asp=0.5, echo=FALSE}
main_data %>%
  mutate(t_essays_n_words = case_when(
    t_essays_n_words == 0 ~ "0",
    t_essays_n_words <= 5 ~ "0-5",
    t_essays_n_words <= 6 ~ "5-6",
    t_essays_n_words <= 7 ~ "6-7",
    TRUE ~ "7+"
  ))  %>%
  group_by(t_essays_n_words, education_kind_working) %>%
  summarise(pct_cats = mean(pets == "cats"), n = n()) %>% ungroup() %>%
  mutate(across(c("t_essays_n_words", "education_kind_working"), as.factor)) %>%
  ggplot(aes(t_essays_n_words, pct_cats, color = education_kind_working)) +
  geom_line(aes(group = education_kind_working)) + geom_point(aes(size = n)) +
  scale_y_continuous(labels = percent_format()) +
  theme_light()
```
...
```{r, echo=FALSE}
cv_res_int <- read_rds("../data/Ok_int_cv_res_auc3.rds")
...
cv_res_int %>%
  ...
  ggplot(aes(recipe, auc, group = id, color = id)) +
  geom_line() + geom_line(data = cv_res_int_meds, color = "black", alpha = 0.2, linewidth = 2) +
...
.insight[
`r emoji("bulb")` I can think of at least 5 semi-random quantitative decisions so far, can you?
]
...
```{r Ok-Test-ROC-Curve, out.width="60%", warning=FALSE, message=FALSE}
roc_lr <- roc_curve(
...
- `r emoji("heavy_check_mark")` Reproducible model
- `r emoji("heavy_check_mark")` Dependable model
- `r emoji("heavy_check_mark")` Explainable model
- `r emoji("heavy_check_mark")` Small and Fast as hell model .font80percent[(put in on a nanochip!)]
- `r emoji("cross_mark")` Impressive model
...
.insight[
`r emoji("bulb")` What *didn't* we do?
]
