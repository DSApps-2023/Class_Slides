...
```{r}
main_data <- rec_eng_commonsense %>% prep(ok_tr_combined) %>% bake(new_data = NULL)
...
```{r Ethnicity-Interaction, out.width="90%", fig.asp=0.5}
main_data %>% group_by(ethnicity_other, location_other) %>%
  summarise(pct_cats = mean(pets == "cats"), n = n()) %>% ungroup() %>%
  mutate(across(c("ethnicity_other", "location_other"), as.factor)) %>%
...
main_data %>%
  ...
  summarise(pct_cats = mean(pets == "cats"), n = n()) %>% ungroup() %>%
  mutate(across(c("speaks_n_fluently", "offspring_one_kid"), as.factor)) %>%
...
```{r Religion-Interaction, out.width="90%", fig.asp=0.5}
main_data %>%
  group_by(religion_laughing, ethnicity_white) %>%
  summarise(pct_cats = mean(pets == "cats"), n = n()) %>% ungroup() %>%
  mutate(across(c("religion_laughing", "ethnicity_white"), as.factor)) %>%
...
rec_int_sex <- recipe(pets ~ ., data = ok_tr_interaction2) %>%
  ...
  step_impute_mean(income) %>%
  step_other(
    all_nominal_predictors(), -has_role("discarded"),
...
rec_int_topints <- recipe(pets ~ ., data = ok_tr_interaction2) %>%
  ...
  step_impute_mean(income) %>%
  step_other(
    all_nominal_predictors(), -has_role("discarded"), -all_outcomes(),
    other = "all_else", threshold = 0.1) %>%
  step_novel(
    all_nominal_predictors(), -has_role("discarded")) %>%
  step_impute_mode(all_nominal_predictors(), -has_role("discarded")) %>%
...
- `r emoji("heavy_check_mark")` Small and Fast as hell model
