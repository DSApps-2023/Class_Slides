...
2. For each node (in parallel): choose the most common label in neighbors
...
The Louvain method is extremely fast, "empirical" complexity appears to be linear. It is also available in NetworkX since v2.7:
...
- In modern Machine Learning, we often get a high-dimensional dataset (say $X_{n\text{x}p}$ and $p$ is very large) some methods cannot handle. One go-to strategy is to reduce the dimension of this dataset matrix by PCA or NMF.
...
In the above example this would be a 10K x 300 matrix where for each word there would be a 300-long *word vector* or *embedding* hopefully representing its semantic/syntactic meaning better than traditional dimensionality reduction methods.
