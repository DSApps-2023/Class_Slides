...
.warning[
`r emoji("warning")` Index is zero based!
]
...
.insight[
`r emoji("bulb")` There *are* Pandas dialects, don't go translating your pipes verbatim.
]
...
Use `apply()` for a more complex function based on multiple columns, to create new column:

```{python}
def tall_and_thin(row):
  thin_body_types = ['skinny', 'fit', 'thin', 'athletic']
  if row['sex'] == 'm':
    avg_height = 180
  else:
    avg_height = 165
  return row['height_cm'] > avg_height and row['body_type'] in thin_body_types

okcupid['is_tall_and_thin'] = okcupid.apply(tall_and_thin, axis=1)

okcupid['is_tall_and_thin'].value_counts()
```
...
Notice we got a `Series` where the index is the grouping variable `sex`.

We could avoid that to get a `DataFrame`:

```{python}
okcupid.groupby('sex', as_index=False)['height_cm'] \
  .agg('mean')
```
...
Feeding `.agg()` with a dictionary would generalize better:

```{python}
okcupid \
  .groupby(['sex', 'status']) \
  .agg({'height_cm': ['mean', 'median', 'count']})
```

But notice the `MultiIndex` for rows and columns.
---
...
I have a much more detailed notebook for you, but for completeness:

```{python Seaborn-Scatterplot, out.width="50%"}
import matplotlib.pyplot as plt
