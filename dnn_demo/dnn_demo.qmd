...
נבחר איזו פונקצית לינק ג'י לתוחלת, לאותה הסתברות, במקרה שלנו הלוג'יט, שהיא לוג יחס הסיכויים, ואת הפונקציה הזאת נמדל באמצעות ליניארי רגיל.
...
בקוד שלפניכם אנחנו מדמים מצב אידיאלי לרגרסיה לוגיסטית עם שני משתנים, וללא חותך. N יהיה שווה 1000 תצפיות, לכן ב-X יהיו 1000 שורות על שתי 

עמודות. וקטור המקדמים האמיתי בטא באורך 2 יהיה פשוט 1, 2.
...
נסמן את וקטור המקדמים שלנו כמטריצה W שיש לה עמודה אחת, נוסיף גם חותך שנקרא לו bias.

במקום להשתמש בגרדיאנט דיסנט על כל הדאטה נשתמש בגרדיאנט דיסנט סטוכסטי או SGD - תיכף נסביר מה זה.

לבסוף נתאר את המודל שלנו בתור דיאגרמה עם חצים ועיגולים, נקרא להם נוירונים.
...
::: {.notes}
::: {style="direction:rtl; font-size:16px"}
אפשר לראות באנטרופיה כמדד למרחק בין שתי התפלגויות. 
...
אפשר לרשום את הנגזרת של Y לפי X לפי כלל השרשרת. הנגזרת של Y לפי X היא הנגזרת שלF לפי G, כפול הנגזרת של G לפי H, כפול הנגזרת של H לפי X.
...
המעבר של Z בפונקצית זיגמויד או ההופכית של G תיקרא פונקציה P של Z.
...
שימו לב שלא כל פונקצית הפסד ניתן בהכרח לפירק לחישוב על מספר באצ'ים. במקרה שלנו ראינו שהלוג-נראות השלילית היא סכום של אלמנטים על כל התצפיות, ולכן היא מתפרקת בפשטות יחסית למספר סכומים, וניתן להראות שזה מספיק כדי לתקף את השינוי הזה.
...
אני מאתחל קלאס SGD עם איזשהו קצב למידה, זהו האלפא שלנו. מקמפל את המודל באמצעות קריאה לmodel.compile(), שם אני מפרט גם את פונקצית ההפסד שהוא האנטרופיה ואת האופטימייזר שלי, הוא הSGD.
